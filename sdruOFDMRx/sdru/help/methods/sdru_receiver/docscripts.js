...
$(document).ready(function () {
    ...
    //Check image sizes
    $('#doc_center_content img, #content_container2 img').each(function () {
        checkImage($(this));
    });
        ...
        if (target) {
            $(this).click(function (evt) {
                ...
                $('html,body').animate({scrollTop:target.offset().top}, 500, function () {
                    ...
                    nextSibling.css('backgroundColor', '#BDEDFF');
                    setTimeout(function () {
                        nextSibling.css('backgroundColor', 'transparent');
                    }, 600);
                });
                location.hash = hash;
            })
...
function setupToc() {
    ...
    var moreEl = $('<img src="/help/search/img/progress.gif" style="position:relative;top:5px;left:5px;" />');
...
function getTocPageId() {
    var pathArray = $(location).attr('pathname').replace(/\/$/,'').split("/");
    var productDir = $(".toc_icon").attr("id").replace(/^toc\./,'');
    var productDirIndex = $.inArray(productDir, pathArray);
    var pageId = "";
    if (productDirIndex === pathArray.length - 1) {
...
function doToggle(elt) {
    var expandable = findExpandableContent(elt);
    expandable.find('.collapse').slideToggle(function () {
        expandable.find('.expand').toggleClass('expanded');
    });
}
