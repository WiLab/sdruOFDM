...
if (isBatInstall)
    incpaths = { fullfile(usrproot,'usrp_uhd_capi','export','include','usrp_uhd_capi') ...
                 fullfile(matlabroot,'src','include') };
else
    incpaths = { fullfile(usrproot,'include') };
end
...
switch (arch)
    case {'glnxa64', 'glnx86'}
        if (isBatInstall)
            ourlibpath = fullfile(matlabroot, 'bin', arch);
        else 
            ourlibpath = fullfile(usrproot, 'bin', arch);
        end
        libnames = {'libmwusrp_uhd_capi.so'};
        
    case {'win32','win64'}
        if (isBatInstall)
            ourlibpath = fullfile(matlabroot, 'lib', arch);
        else
            ourlibpath = fullfile(usrproot, 'lib', arch);
        end
        libnames = {'libmwusrp_uhd_capi.lib'};
    case {'maci64'}
        if (isBatInstall)
            ourlibpath = fullfile(matlabroot, 'bin', arch);
        else 
        ...
        libnames = {'libmwusrp_uhd_capi.dylib'};
  otherwise
   coder.internal.errorIf(true, 'comm:rtwmakecfg:invalidCodeGeneration');      
end

info(1).HostLibFiles    = libnames;
info(1).TargetLibFiles  = libnames;
info(1).IncPaths        = incpaths;
info(1).LibPaths        = { ourlibpath };
info(1).Language        = 'C';
